...
class InsightsDataSourceDocument:
    ...
    def on_update(self):
        ...
        # Với Google Sheet, credentials_changed không quan trọng,
        # nhưng để đơn giản vẫn để như cũ: nếu Active và đổi config → update_table_list
    ...
    def has_credentials_changed(self):
        ...
        elif self.database_type == "Google Sheet":
            # Kiểm tra connection_string đổi hay không
            before = doc_before.connection_string or ""
            after = self.connection_string or ""
            return before != after
    ...
    def validate(self):
        ...
        if self.database_type in ["SQLite", "DuckDB"]:
            ...
            # Config nằm trong connection_string, không cần host/port/user
            return
        else:
...
class InsightsDataSourcev3(InsightsDataSourceDocument, Document):
    ...
    if TYPE_CHECKING:
        ...
        database_type: DF.Literal["MariaDB", "PostgreSQL", "SQLite", "DuckDB", "BigQuery", "ClickHouse", "Google Sheet"]
    ...
    def _get_ibis_backend(self) -> BaseBackend:
        # Google Sheet không dùng backend SQL → không đi vào hàm này
        if self.database_type == "Google Sheet":
            frappe.throw("Google Sheet connection không dùng ibis backend trực tiếp")
    ...
    def _get_db_connection(self):
        ...
        if self.database_type == "Google Sheet":
            from insight_gsheet_connector.insight_gsheet_connector.gsheet_source import (
                get_google_sheet_connection,
            )

            return get_google_sheet_connection(self)

        frappe.throw(f"Unsupported database type: {self.database_type}")

    def get_table_list(self):
        # ✅ Google Sheet: chỉ sync các sheet user đã chọn
        if self.database_type == "Google Sheet":
            ...
            if self.connection_string:
                try:
                    cfg = json.loads(self.connection_string or "{}")
                except Exception:
                    cfg = {}

            # Nếu trong connection_string có "sheets": chỉ sync mấy sheet này
            selected = cfg.get("sheets")
            if selected:
                return selected

            # Nếu chưa chọn sheet cụ thể → trả toàn bộ list để user sync
            return conn.list_tables()

        # Các DB khác giữ nguyên
        ...
        if not database_name or self.database_type in ["DuckDB", "SQLite"]:
    ...
    def update_table_list(self, force=False):
        ...
        # ✅ Với Google Sheet: chỉ tạo table cho các sheet đã chọn (remote_tables)
    ...
    def update_table_links(self, force=False):
        links = []
        # Google Sheet: không có foreign key tự động → bỏ qua
    ...
    def get_ibis_table(self, table_name):
        # ✅ Google Sheet: lấy memtable cho đúng sheet
        if self.database_type == "Google Sheet":
            conn = self._get_db_connection()
